...
* We should be able to do the following with our car:
  * `Car.sell(newOwner)`
    ...
    * This takes a 1 string parameter for the new owner's name.
  ...
  * `Car.paint(newColor)`
    ...
    * This takes a 1 string parameter for the new color's name.
...
* `Car.park()`
  * Only if the car is not running should you console.log `parked!!`.
  * Should return `true` if it is successful and `false` if it is not.
...
* `Car.dropOff(name)`
  * Should take a `name` and remove it from the `passengers` array, but only if it is in the array.
  * Should also only drop passengers off if the car is `on`.
...
**NOTE:** When deciding if there are enough seats available, remember that the driver takes up one seat, but is NOT counted as a passenger in passengerCount(). You can assume the driver is the owner.
